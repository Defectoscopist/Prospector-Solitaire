...
public class Layout : MonoBehaviour
{
    ...
    public PT_XMLHashtable xml; // Для ускорения доступа к xml
    ...
    public void ReadLayout(string xmlText)
    {
        ...
        xml = xmlr.xml["xml"][0]; // Определяется zml для быстрого доступа к XML

        // Прочитать множетели, определяющее расстояние между картами
        ...
        {
            tSD = new SlotDef();
            if (slotsX[i].HasAtt("type")) // если <slot> имеет атрибут type
            {
                tSD.type = slotsX[i].att("type"); // прочитать его
            }
            else
            {
                tSD.type = "slot"; // иначе определить как отдельную карту в ряду
            }
